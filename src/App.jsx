...
async function fetchSheetsGads(cfg, preco, precoUpsell=210) {
  let rows = [];
  ...
  // Busca aba principal e aba de upsell em paralelo
  const upsellUrl = cfg.csvUrl.replace('gid=0', 'gid=819681736');
  const [res, resUp] = await Promise.all([fetch(cfg.csvUrl), fetch(upsellUrl)]);
  if (!res.ok) throw new Error(`CSV: erro ${res.status}`);
  const text = await res.text();
  rows = parseCSV(text).slice(1);
  // Monta Set de emails da aba upsell
  const upsellEmails = new Set();
  if (resUp.ok) {
    const upText = await resUp.text();
    const upRows = parseCSV(upText).slice(1);
    for (const r of upRows) {
      const email = (r[1] || "").trim().toLowerCase();
      if (email) upsellEmails.add(email);
    }
  }
  return rows
    .filter(r => {
...
function crunchAll(metaAds, salesRows, from, to) {
  ...
  function calc(subset, spend, leads) {
    ...
    const upsellRevCap = byCap.filter(s=>s.is_up).reduce((a,s)=>a+(s.upsellValue||0),0);
    const upsellRevSale= bySale.filter(s=>s.is_up).reduce((a,s)=>a+(s.upsellValue||0),0);
    ...
    return {
      ...
      avgDays, daysCount,
      upsellCap, upsellSale, upsellRevCap, upsellRevSale,
    };
  }
...
async function fetchSheets(cfg,preco,precoUpsell=210){
  let rows=[];
  if(cfg.csvUrl){
  ...
  if (cfg.csvUrl) {
    try {
      const upsellUrl = cfg.csvUrl.replace('gid=0', 'gid=819681736');
      const resUp = await fetch(upsellUrl);
      if (resUp.ok) {
        const upText = await resUp.text();
        const parseCSV2=(str)=>{
          const lines=str.trim().split("\n");
        ...
        const upRows = parseCSV2(upText).slice(1);
        for (const r of upRows) {
          const email = (r[1] || "").trim().toLowerCase();
          if (email) upsellEmails.add(email);
        }
      }
    } catch(e) { /* falhou silenciosamente — upsell fica vazio */ }
  }
  return rows
    .filter(r=>{
  ...
  return(
    <>
      <tr className="adrow" onClick={()=>setOpen(o=>!o)} style={{
        ...
        <td style={{padding:"9px 12px",textAlign:"right",fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:C.purple}}>{(isCap?adset.upsellRevCap:adset.upsellRevSale)>0?fmt.brl(isCap?adset.upsellRevCap:adset.upsellRevSale):<span style={{color:C.muted}}>—</span>}</td>
  ...
  return(
    <>
      <tr className="row-camp" onClick={()=>setOpen(o=>!o)} style={{
        ...
        <td style={{padding:"13px 14px",textAlign:"right",fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:C.purple}}>{(isCap?camp.upsellRevCap:camp.upsellRevSale)>0?fmt.brl(isCap?camp.upsellRevCap:camp.upsellRevSale):<span style={{color:C.muted}}>—</span>}</td>
                ...
                <thead style={{position:"relative",zIndex:10}}>
                  <tr style={{borderBottom:`1px solid ${C.border}`}}>
                    ...
                    {["Conjunto · web_utm_term","Gasto","Leads","CPL","CTR","Faturamento","Vendas","Upsell","Lead→Venda"].map((h,i)=>(
                      <th key={Array.isArray(h)?h.join(""):h} style={{padding:"6px 12px",fontSize:8,letterSpacing:1.5,textTransform:"uppercase",color:h==="Upsell"?C.purple+"bb":C.muted,fontFamily:"'JetBrains Mono',monospace",textAlign:i===0?"left":"right",whiteSpace:"nowrap"}}>{h}</th>
                    ))}
...
function crunchGads(gadsRows, salesRows, allSalesRows, allGadsRows, preco, from, to) {
  ...
  // salesMap: Vendas Totais (sale_date no período) e Vendas Geradas (sale_date E capture_date no período)
  const salesMap = {};
  for (const s of (allSalesRows||salesRows)) {
    ...
    if (!salesMap[c]) salesMap[c] = { salesTotal:0, revTotal:0, upsellRevTotal:0, sales15D:0, rev15D:0, salesGeradas:0 };
    salesMap[c].salesTotal++;
    salesMap[c].revTotal += s.value || preco;
    salesMap[c].upsellRevTotal += s.upsellValue || 0;
  ...
  const totalRev       = Object.values(salesMap).reduce((s,r)=>s+r.revTotal,0);
  const totalUpsellRev = Object.values(salesMap).reduce((s,r)=>s+(r.upsellRevTotal||0),0);
  ...
  return { campMap, salesMap, allDatesMap, totalSpend, totalLeads, totalSales, totalRev, totalUpsellRev, totalCpl, totalGeradas };
}
...
// ── Diario Panel ──────────────────────────────────────────────────────────────
function DiarioPanel({ cfg, preco, precoUpsell=210 }) {
    ...
    try {
      ...
      if (useAPI) {
        const [periodData, salesData] = await Promise.all([
          ...
          cfg.csvUrl ? fetchSheetsGads(cfg, preco, precoUpsell) : Promise.resolve([]),
        ]);
        gads = periodData;
        sales = salesData;
      } else {
        const [g, s] = await Promise.all([fetchGadsReport(GADS_URL), cfg.csvUrl ? fetchSheetsGads(cfg, preco, precoUpsell) : []]);
        gads = g; sales = s;
      }
  ...
  const { totalSpend, totalLeads, totalSales, totalRev, totalUpsellRev, totalCpl, totalGeradas, campMap, salesMap, allDatesMap } = crunchGads(filteredGads, filteredSales, salesRows, allGadsRows.length > 0 ? allGadsRows : gadsRows, preco, from, to);
  ...
  // Totais recalculados apenas das campanhas visíveis na tabela (têm gasto no período)
  const visTotalSales    = campaigns.reduce((s,c)=>s+(salesMap[c.campaign]?.salesTotal||0),0);
  const visTotalGeradas  = campaigns.reduce((s,c)=>s+(salesMap[c.campaign]?.salesGeradas||0),0);
  const visTotalRev      = campaigns.reduce((s,c)=>s+(salesMap[c.campaign]?.revTotal||0),0);
  const visTotalUpsell   = campaigns.reduce((s,c)=>s+(salesMap[c.campaign]?.upsellRevTotal||0),0);
  ...
  return (
    <div className="fade-up">
      ...
      {/* KPIs */}
      {(()=>{
        const totRevGads = visTotalRev + visTotalUpsell;
        const lucroGads  = totRevGads - totalSpend;
        const lucroColor = lucroGads >= 0 ? C.green : C.red;
        const roasAcum   = totalSpend > 0 ? totRevGads / totalSpend : null;
        return (
          <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
            ...
            <div style={{flex:"1.2 1 0",minWidth:200,background:C.card,border:`1.5px solid ${C.border2}`,borderRadius:8,padding:"14px 16px",display:"flex",flexDirection:"column",gap:6}}>
              ...
              <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:8}}>
                ...
                <span style={{fontSize:14,fontWeight:700,color:C.green,fontFamily:"'JetBrains Mono',monospace"}}>+{fmt.brl(visTotalRev)}</span>
              </div>
              {visTotalUpsell>0&&(
                <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:8,marginTop:-2}}>
                  <span style={{fontSize:12,fontWeight:700,color:C.purple,fontFamily:"'Plus Jakarta Sans',sans-serif",whiteSpace:"nowrap"}}>Upsell:</span>
                  <span style={{fontSize:14,fontWeight:700,color:C.purple,fontFamily:"'JetBrains Mono',monospace"}}>+{fmt.brl(visTotalUpsell)}</span>
                </div>
              )}
            ...
            <div style={{flex:"1.5 1 0",minWidth:200,background:"#f8fafc",border:`1px solid ${C.border}`,borderRadius:6,padding:"14px 16px",display:"flex",gap:20}}>
              <div>
                ...
                <div style={{fontSize:20,fontWeight:700,color:C.blue,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>{fmt.num(visTotalGeradas)}</div>
                <div style={{fontSize:9,color:C.muted,fontFamily:"'JetBrains Mono',monospace",marginTop:3}}>{fmt.brl(visTotalGeradas * preco)}</div>
              </div>
              <div style={{width:1,background:C.border}}/>
              <div>
                <div style={{fontSize:8,letterSpacing:2,textTransform:"uppercase",color:C.muted,fontFamily:"'JetBrains Mono',monospace",marginBottom:3}}>Vendas Totais</div>
                <div style={{fontSize:20,fontWeight:700,color:C.green,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>{fmt.num(visTotalSales)}</div>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
            <thead style={{position:"relative",zIndex:10}}>
              <tr style={{borderBottom:`1px solid ${C.border}`}}>
                {[
                  {h:"Campanha",      tip:"Nome da campanha no Google Ads",       br:false},
                  {h:"Gasto",         tip:"Gasto no período selecionado",          br:false},
                  {h:"Leads",         tip:"Leads captados no período selecionado", br:false},
                  {h:"CPL",           tip:"Custo por Lead = Gasto ÷ Leads",        br:false},
                  {h:["Vendas","Geradas"],    tip:"Vendas cujo lead foi captado E comprou dentro do período selecionado", br:true},
                  {h:["Vendas","Totais"],     tip:"Todas as vendas com data de compra no período, independente de quando o lead foi captado", br:true},
                  {h:"Upsell",             tip:"Receita incremental de upsell no período", br:false},
                  {h:"Lucro Total",        tip:"Faturamento Acumulado + Upsell menos o Gasto do período", br:false},
                ].map(({h,tip,br},i)=>(
                  <th key={Array.isArray(h)?h.join(""):h} style={{padding:"9px 14px",textAlign:i===0?"left":"center",fontSize:8,letterSpacing:1.5,textTransform:"uppercase",color:C.muted,fontFamily:"'JetBrains Mono',monospace",lineHeight:1.4,verticalAlign:"bottom"}}>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:0}}>
                      {br?(Array.isArray(h)?h:[h]).map((l,j)=><span key={j}>{l}</span>):<span style={{whiteSpace:"nowrap"}}>{h}</span>}
                      <Tip text={tip}/>
                    </div>
                  </th>
                ))}
                ...
                <th style={{padding:"9px 14px",textAlign:"right",fontSize:8,letterSpacing:1.5,textTransform:"uppercase",color:C.gold+"aa",fontFamily:"'JetBrains Mono',monospace",whiteSpace:"nowrap"}}>ROAS ACUMULADO<Tip text="Vendas Totais × preço ÷ Gasto do período"/></th>
              ...
              {campaigns.map((camp, i) => {
                ...
                return (
                  <tr key={camp.campaign} style={{borderBottom:`1px solid ${C.border}`,background:i%2?"#f8fafc":"transparent"}}>
                    ...
                    <td style={{padding:"11px 14px",textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:12}}>{fmt.num(sm.salesGeradas||0)}</td>
                    <td style={{padding:"11px 14px",textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:12,fontWeight:600}}>{fmt.num(sm.salesTotal||0)}</td>
                    <td style={{padding:"11px 14px",textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:C.purple}}>{(sm.upsellRevTotal||0)>0?fmt.brl(sm.upsellRevTotal):<span style={{color:C.muted}}>—</span>}</td>
                    <td style={{padding:"11px 14px",textAlign:"right",fontFamily:"'JetBrains Mono',monospace",fontSize:12,fontWeight:600,color:(sm.revTotal||0)+(sm.upsellRevTotal||0) - camp.spendPeriod >= 0 ? C.green : C.red}}>{fmt.brl((sm.revTotal||0)+(sm.upsellRevTotal||0) - camp.spendPeriod)}</td>
...
export default function Dashboard(){
  ...
  const load=useCallback(async(c,f,t,p,pu=210)=>{
    ...
    try{
      const [meta,sales]=await Promise.all([fetchMetaAds(c,f,t),fetchSheets(c,p,pu)]);
  ...
  const apply=()=>{
    load(cfg,from,to,preco,precoUpsell);
  };

  useEffect(() => { load(cfg, from, to, preco, precoUpsell); }, []);
  ...
  const totUpsellRevCap =rows.reduce((s,r)=>s+(r.upsellRevCap||0),0);
  const totUpsellRevSale=rows.reduce((s,r)=>s+(r.upsellRevSale||0),0);
        ...
        {page==="cfg"?(
          <div className="fade-up">
            ...
            <div style={{padding:"22px 0"}} className="fade-up">
              <DiarioPanel cfg={cfg} preco={preco} precoUpsell={precoUpsell}/>
            </div>
          ):tab==="semanal"?(
            <div style={{padding:"22px 0"}} className="fade-up">
          ...
          <div className="fade-up">
                ...
                {[
                  ...
                  return (
                    <button key={l} onClick={()=>{
                      ...
                      load(cfg,f,t,preco,precoUpsell);
                    }} style={{
                      padding:"4px 12px",
            ...
            {(()=>{
              ...
              const upRev   = isCap ? totUpsellRevCap : totUpsellRevSale;
              const sales   = isCap ? totSalesCap : totSalesSale;
              const totRev  = rev + upRev;
              const roas    = totSpend>0&&totRev>0 ? totRev/totSpend : null;
              const lucro   = totRev - totSpend;
                ...
                <div className="kpi-section" style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
                  ...
                  <div className="kpi-financial" style={{flex:"1.2 1 0",minWidth:200,background:C.card,border:`1.5px solid ${C.border2}`,borderRadius:8,padding:"14px 16px",display:"flex",flexDirection:"column",gap:6}}>
                    ...
                    {upRev>0&&(
                      <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:8,marginTop:-2}}>
                        <span style={{fontSize:12,fontWeight:700,color:C.purple,fontFamily:"'Plus Jakarta Sans',sans-serif",whiteSpace:"nowrap"}}>Upsell:</span>
                        <span style={{fontSize:14,fontWeight:700,color:C.purple,fontFamily:"'JetBrains Mono',monospace"}}>+{fmt.brl(upRev)}</span>
                      </div>
                    )}
                    <div style={{height:1,background:C.border,margin:"2px 0"}}/>
                    <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:8}}>
